import { Receipt, Copy, Share2, TrendingUp } from 'lucide-react';
...
export default function SummarySection({ products, people, totals, tipType, tipValue, bankData, currency }: Props) {
  ...
  const fmtCLP = (n: number) => formatCurrency(n * exchangeRate, 'CLP');
    ...
    <section className="rounded-2xl bg-card p-5 card-shadow-lg animate-fade-in-up border border-primary/20">
      ...
      <div className="flex justify-between items-center mb-5 text-base border-t border-border pt-3 mt-2">
        <div className="flex flex-col">
          ...
          {showConversion && currency === 'BRL' && (
            <span className="text-xs text-muted-foreground font-medium">Equiv. {fmtCLP(grandTotal)}</span>
          )}
        </div>
        ...
        <div className="mb-6 p-3.5 bg-primary/5 rounded-2xl border border-primary/10">
          ...
          {showConversion && (
            <div className="flex items-center gap-3 animate-fade-in">
              <span className="text-xs text-muted-foreground font-medium whitespace-nowrap">1 Real =</span>
              <div className="relative flex-1">
                <span className="absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground text-[10px] font-bold">$</span>
                <Input 
                  type="number"
                  value={exchangeRate}
              ...
              <span className="text-xs text-muted-foreground font-medium">Pesos</span>
            </div>
          )}
        </div>
      )}
            ...
            <div key={person.id} className="bg-accent/50 rounded-xl p-3.5 animate-scale-in">
              <div className="flex items-center gap-2.5 mb-2">
                ...
                <div className="flex flex-col items-end">
                  ...
                  {showConversion && currency === 'BRL' && (
                    <span className="text-[10px] text-primary font-bold mt-1">~ {fmtCLP(pt.total)}</span>
                  )}
                </div>
              </div>
